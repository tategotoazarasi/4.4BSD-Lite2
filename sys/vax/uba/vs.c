...
int flag;
{
	...
	vsBuffnpages = (((int) vsb & PGOFSET) +
	                (NBPG - 1) + sizeof(struct vsBuffArea)) >>
	               PGSHIFT;
	...
	vsb->vsioa.reloc = (int) (vsp->offset.fparm_all + UBAI_ADDR(vsp->vsBuff_ubinfo));
	return (0);
}
        ...
        dev_t dev;
{
	...
	if(VSUNIT(dev) >= NVS || (uip = vsdinfo[VSUNIT(dev)]) == 0 || uip->ui_alive == 0) {
